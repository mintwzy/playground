class PhotosController < ApplicationController
  ...
  def random
    photos = Photo.order("RAND()").limit(params[:count])
    res = photos.map{|photo| {path: photo.path}}
  ...
  def generate
    patterns = "#{ENV['PHOTO_PATH']}/*/**/*.jpg"
    data = Dir.glob(patterns).map{|path| {path: path.split('/').drop(8).join('/')}}
    Photo.insert_all!(data)
